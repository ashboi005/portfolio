...
- **Language** — English or Hinglish, picked on the idle screen and remembered in localStorage. It drives both halves: the coach's register (Roman script, jargon left in English) *and* Deepgram's language, since someone drilling in Hinglish will speak Hinglish and `en` would mangle it. Hinglish uses nova-3's `multi` code-switching mode, which costs the English-only keyterm boost — the UI says so.
