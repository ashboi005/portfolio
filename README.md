...
- **Grading** — a *second* AutoSage agent (one-shot, never given a `chat_id`) returns a rubric: correctness, depth, structure, signal/noise, plus a verdict, what you skipped, and what he'd have asked next. Setup and the system prompt live in `docs/drill-coach-agent-prompt.md`.
...
The concept bank is `apps/server/src/data/concepts.json` — 319 concepts across three levels, each with `probes` that double as on-screen hints and the grading bar sent to the agent.
